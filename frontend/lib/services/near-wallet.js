...
export class Wallet {
   ...
   */
  constructor({ networkId, createAccessKeyFor }) {
  ...
  startUp = async (accountChangeHook) => {
    this.selector = setupWalletSelector({
      network: {
        ...
        nodeUrl: "https://rpc.testnet.near.org", // "https://rpc.testnet.pagoda.co",
      },
      modules: [
        setupMyNearWallet(),
        // Disabled other wallets because of the MPC and RPC delay issue.
        // setupHereWallet(),
        // setupMeteorWallet(),
        // setupBitteWallet(),
      ],
   ...
   * @param {Object} options - the options for the call
   * @param {string} options.receiverId - the receiver's account id
   * @param {string} options.deposit - the amount of tokens to transfer
   * @returns {Promise<Transaction>} - the resulting transaction
   */
  transfer = async ({ receiverId, deposit }) => {
    const selectedWallet = await (await this.selector).wallet();
    const outcome = await selectedWallet.signAndSendTransaction({
      receiverId,
      actions: [
