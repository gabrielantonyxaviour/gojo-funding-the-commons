...
impl Contract {
    pub(crate) fn internal_unwrap_balance_of(&self, account_id: &AccountId) -> NearToken {
        match self.accounts.get(account_id) {
            ...
            None => {
                env::panic_str(format!("Account {} is not registered", account_id).as_str())
            }
        }
    }
    ...
    pub(crate) fn measure_bytes_for_longest_account_id(&mut self) {
        ...
        let tmp_account_id: AccountId = format!("{}", "a".repeat(64)).parse().unwrap();
        self.accounts.insert(&tmp_account_id, &ZERO_TOKEN);
        self.bytes_for_longest_account_id = env::storage_usage() - initial_storage;
